    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Section title="Basic Information">
          ...
          <Field label="Category" id="category"><select id="category" required value={form.category} onChange={set('category')} className="input-field">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></Field>
        ...
        <Section title="Location & Assignment">
          <Field label="Branch" id="branch"><select id="branch" required value={form.branch} onChange={set('branch')} className="input-field">{BRANCHES.map((b) => <option key={b} value={b}>{b}</option>)}</select></Field>
          <Field label="Department" id="department"><select id="department" required value={form.department} onChange={set('department')} className="input-field">{DEPARTMENTS.map((d) => <option key={d} value={d}>{d}</option>)}</select></Field>
